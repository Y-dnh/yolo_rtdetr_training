...
2. **Дописування Albumentations**
   - `config["augmentations"] = CUSTOM_TRANSFORMS` — у конфіг додається глобальний список трансформ.

3. **Логування аугментацій**
   - Опис аугментацій зберігається в теку run: `runs/<project>/<name>/augmentations_info.yaml` (або `..._resume.yaml` при resume). У `args.yaml` Ultralytics кастомний список `augmentations` не записує.
...
TRAINING_CONFIG + AUGMENTATION_CONFIG + kwargs
        ...
        → _save_augmentations_info(config)
        → load_model(path)
        → model.train(**config)
...
└── runs/                    # Усі результати (тренування + валідація)
    └── <PROJECT_NAME>/      # Назва проєкту (наприклад yolo26m, yolo26s_p2)
        ...
        │   │   │   └── best.onnx
        │   │   ├── args.yaml
        │   │   ├── augmentations_info.yaml  # або augmentations_info_resume.yaml
...
- `<PROJECT_NAME>/baseline/args.yaml` — конфіг запуску (ultralytics; без списку `augmentations`)
- `<PROJECT_NAME>/baseline/augmentations_info.yaml` — опис використаних Albumentations (назви та параметри трансформ)
